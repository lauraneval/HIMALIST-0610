...
export default function StudioList(){
    ...
    useEffect(() => {
        const fetchStudio = async () => {
            const { data: studioData, error: studioError } = await createClient().from('studio').select('*').order('name', { ascending: true });
            ...
            <div className="mb-6 w-full flex justify-between items-center">
                ...
                <Dialog open={createDialog} onOpenChange={setCreateDialog}>
                    <DialogTrigger asChild>
                        <Button className="font-bold bg-secondary hover:bg-secondary-hover text-foreground">Add Studio</Button>
                    </DialogTrigger>
                    ...
                    <TableBody>
                        {studios.length === 0 ? (
                            ...
                            studios.map((studio: Studio) => (
                                <TableRow key={studio.id} className="border-b border-border">
